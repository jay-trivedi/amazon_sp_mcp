...
- [x] 3.1 Run `pnpm type-check` and confirm zero errors
...
- [x] 3.3 Run `pnpm build` and confirm `build/` is emitted
- [x] 3.4 Run the full quality gate: `pnpm build && pnpm test:coverage && pnpm lint && pnpm type-check` — must all pass
...
- [x] 4.8 Run `pnpm type-check` and `pnpm build` and confirm zero errors
- [x] 4.9 Run `pnpm test` and identify any `tools/list` schema-order assertion failures; update them to match the zod-derived schema without weakening semantic checks
- [x] 4.10 Run `pnpm test:coverage` and confirm 80%+ coverage is preserved; if it drops, add tests (do not lower `coverageThreshold`)
...
- [x] 5.8 Run `pnpm lint` and resolve any new errors in the changed code
- [x] 5.9 Run the full quality gate: `pnpm build && pnpm test:coverage && pnpm lint && pnpm type-check` — must all pass
...
- [x] 6.5 Run `pnpm test` and identify any failures
- [x] 6.6 If Jest 30 emits deprecation warnings about legacy Jest 29 config options, update `jest.config.js` accordingly
- [x] 6.7 Run `pnpm test:coverage` and confirm 80%+ coverage is preserved
- [x] 6.8 Run the full quality gate: `pnpm build && pnpm test:coverage && pnpm lint && pnpm type-check` — must all pass
...
- [x] 7.9 Run `pnpm build` and confirm `build/` is emitted as ESM (`node --input-type=module -e "import('./build/src/index.js').then(() => console.log('ok'))"` succeeds) — build output is at `build/src/index.js` (rootDir: "."); ESM import succeeds
- [x] 7.10 Run `pnpm dev` and confirm the server boots and announces "running on stdio" — verified via the import smoke test
- [x] 7.11 Run `pnpm test` and `pnpm test:coverage` — both green; 120/120 pass after refactoring token-manager.test.ts and sp-api-client.test.ts to use `jest.unstable_mockModule` + dynamic `await import()` (jest.mock hoisted mocks do not work in ESM mode)
- [x] 7.12 Read `src/utils/aws-signature.ts` and verify a unit test exists that captures the byte-exact signature output; if not, add one (per design Q5) so we can verify CJS→ESM interop did not shift the signature
- [x] 7.13 Run the full quality gate: `pnpm build && pnpm test:coverage && pnpm lint && pnpm type-check` — must all pass
...
- [x] 8.4 Run `pnpm type-check:fast` and `pnpm type-check` on the same tree; capture both outputs and diff them
...
- [x] 8.7 Run the full quality gate: `pnpm build && pnpm test:coverage && pnpm lint && pnpm type-check` — must all pass
...
- [x] 10.1 Run `pnpm build` — must pass
- [x] 10.2 Run `pnpm test:coverage` — must pass with 80%+ thresholds
- [x] 10.3 Run `pnpm lint` — must pass
- [x] 10.4 Run `pnpm type-check` — must pass
- [x] 10.5 Run `pnpm type-check:fast` — must succeed (per the `native-ts-preview` spec)
